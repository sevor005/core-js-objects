...
function makeWord(lettersObject) {
  if (!Object.keys(lettersObject).length) return '';

  const maxLength = Math.max(
    ...Object.values(lettersObject).map((num) => Math.max(...num))
  );
  let result = '';

  [...Array(maxLength + 1).keys()].forEach((i) => {
    result += Object.entries(lettersObject)
